...
export const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    ...
    if (!hash.includes('type=recovery')) {
      // Not a valid recovery link — redirect to login
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccess(true);
      setTimeout(() => navigate('/hoy', { replace: true }), 2000);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#E8E6DF', padding: 24, fontFamily: 'Arial, sans-serif' }}>
      <div style={{ maxWidth: 420, width: '100%', background: '#F7F6F2', borderRadius: 16, padding: '48px 40px', boxShadow: '0 32px 80px rgba(13,33,55,0.15)' }}>
        ...
        <div style={{ fontFamily: 'Georgia, serif', fontSize: 22, color: '#0D2137', marginBottom: 4 }}>
          Nueva contrasena
        </div>
        ...
        {success ? (
          <div style={{ padding: '16px', background: 'rgba(29,158,117,0.08)', border: '1px solid rgba(29,158,117,0.2)', borderRadius: 8, color: '#1D9E75', fontSize: 14, textAlign: 'center' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              {loading ? (
                ...
                <><KeyRound size={15} /> Guardar contrasena</>
              )}
            </button>
          </form>
        )}
