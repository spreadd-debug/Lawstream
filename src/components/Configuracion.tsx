    ...
    <div className="space-y-10 max-w-3xl mx-auto pb-20">
      ...
      <div className="space-y-4">
        {sections.map(section => (
          <Card key={section.id} className="p-6 flex items-center gap-6 hover:translate-x-2 transition-all group bg-card border-border shadow-lg hover:shadow-xl cursor-pointer">
      ...
      <div className="pt-12 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-6">
        ...
        <Button variant="danger" size="sm" className="w-full sm:w-auto gap-2 text-[10px] font-black uppercase tracking-widest px-8 rounded-xl shadow-lg shadow-destructive/10">
          <LogOut size={18} />
          Cerrar Sesión
        </Button>
      </div>
